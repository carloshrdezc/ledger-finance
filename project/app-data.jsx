// Shared sample data, formatters, and state hooks for both directions.
...
// Hierarchical category tree — categories can nest arbitrarily deep.
// Paths are passed around as arrays (['edu','school','supplies','pencils'])
// and rendered as breadcrumbs ("EDUCATION › SCHOOL › SUPPLIES › PENCILS").
const CATEGORY_TREE = {
...
// Resolve a path (array) to its leaf label, or return null if invalid.
...
// Day offsets from "today" for the demo. ~30 transactions across categories.
// `path` is the full hierarchical category path (array). `cat` is path[0],
...
// Top-merchant aggregation for reports.
const MERCHANTS = [
...
// Month-over-month spend trail for reports (last 12 months).
const MOM_SPEND = [4820, 5102, 4940, 5380, 5210, 5640, 4980, 5340, 5510, 5180, 6713, 5234];
...
// 30 daily net-worth points for the hero sparkline (ascending with noise).
const SPARK_NW = [
...
const SPARK_SPEND = [ // monthly spend trail (last 30d)
  124, 88, 0, 245, 60, 110, 0, 320, 78, 0, 180, 0, 412, 90, 60, 540, 0, 70, 100, 220,
  0, 88, 0, 612, 45, 30, 0, 480, 92, 110,
];

const NET_WORTH = ACCOUNTS.reduce((s, a) => s + (a.ccy === 'USD' ? a.bal : a.bal * 1.08), 0);
const MONTH_SPEND = TRANSACTIONS.filter(t => t.cat !== 'income' && t.amt < 0).reduce((s, t) => s + Math.abs(t.ccy === 'USD' ? t.amt : t.amt * 1.08), 0);
const CASH = ACCOUNTS.filter(a => ['CHK','SAV','FX'].includes(a.type)).reduce((s, a) => s + (a.ccy === 'USD' ? a.bal : a.bal * 1.08), 0);
const SAFE_DAY = 162.50; // demo number
...
const HERO_METRICS = [
  ...
  { key: 'safe',  label: 'SAFE TO SPEND',  value: SAFE_DAY,    delta: -32.10,   deltaPct: -16.5, spark: SPARK_NW.map(v=>v*0.0006), ccy: 'USD', unit: '/ DAY' },
];

// ─── formatters ─────────────────────────────────────────────────────────
const CCY_SYM = { USD: '$', EUR: '€', GBP: '£' };
function fmtMoney(v, ccy = 'USD', decimals = true) {
...
function fmtSigned(v, ccy = 'USD', decimals = true) {
  const sym = CCY_SYM[ccy] || '$';
  const abs = Math.abs(v);
  const txt = decimals
    ? abs.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
...
Object.assign(window, {
  ACCOUNTS, CATEGORIES, CATEGORY_TREE, TRANSACTIONS, BUDGETS, GOALS, BILLS, INVESTMENTS,
  MERCHANTS, MOM_SPEND,
  HERO_METRICS, NET_WORTH, MONTH_SPEND, CASH, SAFE_DAY,
  SPARK_NW, SPARK_SPEND, CCY_SYM,
  fmtMoney, fmtSigned, fmtPct, dateForOffset, dayLabel, TODAY,
  catLabel, catGlyph, catBreadcrumb,
});
